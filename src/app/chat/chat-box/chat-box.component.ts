...
export class ChatBoxComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.currentUser=this.AppService.getUserInfoFromLocalstorage()["userId"];
  ...
  public checkStatus: any = () => {
    ...
    if (Cookie.get('authtoken') === undefined || Cookie.get('authtoken') === '' || Cookie.get('authtoken') === null) {
      ...
      alert("Check status in chat box");
  ...
  public getOnlineUserList :any =()=>{
    ...
    this.SocketService.onlineUserList()
      .subscribe((userList) => {
        console.log("getOnline user list in chat box");
        console.log(userList);
        this.userList = [];

        for (let x in userList) {

          if(x!=this.currentUser){
    ...
    public userSelectedToChat: any = (id, name) => {
     ...
     return user
      ...
      this.messageList = []; //About chat-page refreshing so that previos chat user page is removed.
    ...
    public sendMessageUsingKeypress: any = (event: any) => {
      ...
      if (event.keyCode === 13 || event.keyCode==65) { // 13 is keycode of enter.
